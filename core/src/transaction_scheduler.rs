...
impl TransactionScheduler {
    ...
    fn try_schedule(
...

        {
            ...
            // TODO what else does banking stage do to filter transactions? should do that here
            //  so batch is guaranteed to be ok
            // - run QoS (cost model)
            // - lock accounts with QoS results
            // - check blockhash age
            // - check to make sure it's not already processed
            // - check fee payer has a high enough balance
            //
            // if cost model is exceeded, throw it in another buffer

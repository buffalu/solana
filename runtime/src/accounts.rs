        ...
        if tx.signatures().is_empty() && fee != 0 {
            ...
            for (i, key) in account_keys.iter().enumerate() {
                let account = if !message.is_non_loader_key(i) {
                    ...
                    let mut account = if solana_sdk::sysvar::instructions::check_id(key) {
                    ...
                    // Run payer balance check as soon as the payer account is loaded.
                    // If the fee payer doesn't have enough lamports, bypass loading the rest
                    // of the accounts.
                    if payer_index.is_none() {
                        payer_index = Some(i);
                        if payer_index != Some(0) {
                        ...
                        let min_balance =
                            match get_system_account_kind(&account).ok_or_else(|| {
                        ...
                        if account.lamports() < fee + min_balance {
                            error_counters.insufficient_funds += 1;
                            return Err(TransactionError::InsufficientFundsForFee);
                        }
                        let payer_pre_rent_state =
                            RentState::from_account(&account, &rent_collector.rent);
                        account
